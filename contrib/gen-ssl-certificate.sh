...
echo "copying root.pem, server.crt and server.key to ../data/cert"
mkdir -p data/cert
cp $KEYDIR/root.pem data/cert
cp $KEYDIR/server.crt data/cert
cp $KEYDIR/server.key data/cert

echo "exporting client certificates in pkcs12"
for name in grace cecilia; do
    openssl pkcs12 -export -nodes -in $KEYDIR/"$name".crt -inkey $KEYDIR/"$name".key -out $KEYDIR/"$name".p12 -name ""$name"-flask-local" -passout pass:password
done
